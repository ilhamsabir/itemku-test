 ...
 * 
 * @param {any[]} relation - relation document data
...
function solution(relation) {
  ...
  for (const i in relation[0]) {
    relationMap[i] = relation.map(data => data[i])
  }

  // get all possible combination
  const combinedElm = combineArrayElement(Object.keys(relationMap))

  let answer = 0 // the number of candidate keys
  ...
  for (const keys of combinedElm) {
    ...
    if (!keys.some(key => usedKeys.includes(key))) {
      const multiArray = keys.map(key => relationMap[key])
